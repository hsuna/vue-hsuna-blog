...
const mainfastJsCode = `!function(){function b(a){var c,b=document.createElement(a.tag);for(c in a.attrs)b[c]=a.attrs[c];b.children=a.children,document.getElementsByTagName(a.injectTo)[0].appendChild(b)}var c,a=$1;for(c=0;c<a.length;c++)b(a[c])}();`;

// https://gist.github.com/samthor/64b114e4a4f539915a95b91ffd340acc
// DO NOT ALTER THIS CONTENT
const safari10NoModuleFix = `!function(){var e=document,t=e.createElement("script");if(!("noModule"in t)&&"onbeforeload"in t){var n=!1;e.addEventListener("beforeload",(function(e){if(e.target===t)n=!0;else if(!e.target.hasAttribute("nomodule")||!n)return;e.preventDefault()}),!0),t.type="module",t.src=".",e.head.appendChild(t),t.remove()}}();`;

const cacheSearch = '"?_cache="+new Date().getTime()';
const legacyPolyfillId = 'vite-legacy-polyfill';
const legacyEntryId = 'vite-legacy-entry';
const entryJSInlineCode = `!function(){var s=document.createElement("script");s.src="$1"+${cacheSearch};document.getElementsByTagName("head")[0].appendChild(s);}();`;
...
function viteLegacyPlugin(options = {}) {
  ...
  const DEFAULT_LEGACY_POLYFILL = ['core-js/modules/es.promise', 'core-js/modules/es.array.iterator'];
  ...
  const legacyPostPlugin = {
        ...
        presets: [
          ...
          [
            'env',
            {
              ...
              corejs: needPolyfills ? { version: 3, proposals: false } : undefined,
    ...
    transformIndexHtml(html, { chunk, bundle }) {
      ...
      // 6. replace index
      const INDEX_SCRIPT_REG = /<script type="module" crossorigin src="([^"]+)"><\/script>/;
      if (INDEX_SCRIPT_REG.exec(html)) {
        tags.unshift({
          tag: 'script',
          attrs: {
            type: 'module',
            crossorigin: true,
            src: INDEX_SCRIPT_REG.exec(html)[1],
          },
          injectTo: 'head',
        });
        html = html.replace(INDEX_SCRIPT_REG, '');
      }
      const INDEX_CSS_REG = /<link rel="stylesheet" href="([^"]+)"[\S]*\/?>/;
      if (INDEX_CSS_REG.exec(html)) {
        tags.unshift({
          tag: 'link',
          attrs: { rel: 'stylesheet', href: INDEX_CSS_REG.exec(html)[1] },
          injectTo: 'head',
        });
        html = html.replace(INDEX_CSS_REG, '');
      }

      const mainfastFileName = `mainfast${chunk.name !== 'main' ? `.${chunk.name}` : ''}.js`;
      bundle[mainfastFileName] = {
        fileName: mainfastFileName,
        name: mainfastFileName,
        source: mainfastJsCode.replace('$1', JSON.stringify(tags)),
        type: 'asset',
        isAsset: true,
      };
      ...
      return {
        ...
        tags: [
          ...
          {
            tag: 'script',
            children: entryJSInlineCode.replace('$1', `${config.base}${mainfastFileName}`),
            injectTo: 'head',
          },
        ],
      };
  ...
  const legacyEnvPlugin = {
    ...
    config(config, env) {
      if (env) {
        return {
          define: {
            'import.meta.env.LEGACY': env.command === 'serve' || config.build.ssr ? false : legacyEnvVarMarker,
          },
        };
      } else {
  ...
  return [legacyConfigPlugin, legacyGenerateBundlePlugin, legacyPostPlugin, legacyEnvPlugin];
}
